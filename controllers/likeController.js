...
export const like = (req, res) => {
    ...
    if (blog) {
        Blog.findByIdAndUpdate(blogId, { $inc: { likes: 1 } })
            ...
            .catch((err) =>
                res.status(404).json({ error: err, message: "Like not added" })
            );
...
export const unlike = (req, res) => {
    ...
    if (blog) {
        if(blog.likes == 0){
            return res.status(404).json({error: "Can't unlike"});
        }
        Blog.findByIdAndUpdate(blogId, { $inc: { likes: -1 } })
            .then((data) =>
            ...
            .catch((err) =>
                res.status(404).json({ error: err, message: "Like not removed" })
            );
    }
};
